...
public class CustomerMainUI {
	...
	ArrayList<ProductCard> getProductCards(Document invDoc) {
		ObjectId sellerId = invDoc.getObjectId("sellerId");
		...
		for(Document inv : inventoryList) {
			...
			ProductCard pc = new ProductCard(sellerId, productId, productPrice);
			productCards.add(pc);
		}
...
class ProductCard extends VBox {
	...
	public ProductCard(ObjectId sellerId, int productId, int productPrice) {
		super(5);
